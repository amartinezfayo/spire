...
func (c *setCommand) appendFlags(fs *flag.FlagSet) {
	...
	fs.StringVar(&c.format, "format", formatX509, fmt.Sprintf("The format of the bundle data. Either %q or %q.", formatX509, formatJWKS))
}
...
func (c *setCommand) run(ctx context.Context, env *env, clients *clients) error {
	...
	if strings.ToLower(format) == formatX509 {
